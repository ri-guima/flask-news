...
def test_add_page_get(client: FlaskClient) -> None:
    response = client.get(url_for('flask_news.add_page_get'))
    assert response.status_code == 200


def test_add_page_post(client: FlaskClient) -> None:
    response = client.post(url_for('flask_news.add_page_post'))
    assert response.status_code == 302
